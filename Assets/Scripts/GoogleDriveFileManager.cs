...
namespace In.App.Update
{
    public class GoogleDriveFileManager
    {
        ...
        private string tokenPath => Path.Combine(Application.streamingAssetsPath, "token2.json");
        ...
        public async UniTask DownloadFileAsync(string fileId, string saveToPath, Action<float> onProgress = null,Action onComplete=null,Action<string> onFailure=null)
        {
            if(_driveService==null) _driveService= await GetDriveService();
            try
            {
        ...
        public async UniTask<DriveService> GetDriveService()
        {
            ...
            string tokenData =JsonCryptoUtility.DecryptJsonFromFile(Path.Combine(Application.streamingAssetsPath, "token2.json"));
