...
public class BuildManagerWindow : EditorWindow
{
    ...
    private  void DrawVersionControl()
    {
        ...
        for (int i = 0; i < versions.Count; i++)
        {
            ...
            version.isExpanded = EditorGUILayout.Foldout(version.isExpanded, $"v{version.versionName} - {version.releaseTitle}", true);
            ...
            if (version.isExpanded)
            {
                ...
                {
                    if (EditorUtility.DisplayDialog("Confirm Delete", $"Are you sure you want to delete version {version.versionName}?", "Yes", "No"))
                    {
    ...
    private async void DrawBuildManager()
    {
        ...
        {
            EditorGUILayout.HelpBox("Version Name must be in the format 'major.minor.patch' (e.g., 0.1.1).", MessageType.Error);
        }
    ...
    public async void UploadBuild(string buildPath)
    {
        ...
        Google.Apis.Drive.v3.Data.File existingBuildFile=await GoogleDriveFileManager.GetInstance().GetFileByNameAsync(fileName,Application.productName);
        Google.Apis.Drive.v3.Data.File buildFile;
        if(existingBuildFile==null) buildFile= await GoogleDriveFileManager.GetInstance().UploadFileAsync(buildPath, Application.productName);
        else buildFile= await GoogleDriveFileManager.GetInstance().UpdateFileAsync(buildPath,existingBuildFile.Id, Application.productName);
        ...
        existingFile=await GoogleDriveFileManager.GetInstance().GetFileByNameAsync("release_data.json",Application.productName);
        if (existingFile != null) await GoogleDriveFileManager.GetInstance().DownloadFileAsync(existingFile.Id, releaseDataPath, (progress) => { });
        ...
        if(existingFile==null) releaseFile= await GoogleDriveFileManager.GetInstance().UploadFileAsync(releaseDataPath, Application.productName);
        else releaseFile= await GoogleDriveFileManager.GetInstance().UpdateFileAsync(releaseDataPath,existingFile.Id, Application.productName);
    }
    private async UniTask CreateZip(string sourceDirectory, string zipFilePath)
    {
        try
        {
            ...
            await UniTask.Run(() =>
            {
                ...
                MakeExecutable(Path.Combine(Application.dataPath, "..", "TestBuild", "MyGame.app", "Contents",
                    "MacOS"));
